...
- **상품/단가 관리 전면 수정 (9종 상품유형 중심 구조)**: `productType` 9종(translation/interpretation/combined/equipment/project/transport/meal/accommodation/other_cost), `sourceLanguage`/`targetLanguage` 컬럼 분리(languagePair deprecated 유지), 대분류/중분류 자동 변경, 코드 자동생성(언어형: `TR-KO-EN-LAW-001`, 비언어형: `EQ-SIM-001`), 중복체크 API, 상품 등록 요청 워크플로우(pending→approved/rejected). `ProductManagementTab.tsx` 전면 재작성.
...
- **AEO/GEO 자동 보완 + Publish Queue 시스템**: `insight_auto_suggestions` 테이블 (insightId/type/payload/status). AI(GPT)가 부족한 AEO 필드 자동 탐지 → 제안 생성(POST /auto-enhance 단건 / batch). 자동 보완 후 품질 조건(aeoScore≥80, faqCount≥3, relatedCount≥2, filterDecision≠drop) 충족 시 자동으로 `publish_ready` 상태 전환, 미충족 시 `review_ready`. `POST /admin/content-insights/:id/publish`로 게시(status=published, publishedAt 설정, slug 자동 생성). `GET /admin/content-insights/publish-queue`로 게시 대기 목록 조회. 어드민 UI: 상태 탭(전체 상태/초안/검토 준비/게시 대기/게시됨) + 테이블 내 "📤 게시" 버튼 + 상세 패널 "📤 게시하기" 버튼. 상태 배지: review_ready(노란색), publish_ready(보라색), published(파란색).
...
- **Database Schema:** A normalized PostgreSQL schema manages all platform entities. Key entities include `users`, `projects`, `quotes`, `quote_items`, `tasks`, `payments`, `settlements` (with auto-generation fields: taskId, grossAmount, netAmount, settlementType, withholdingAmount, vatAmount, payoutDueDate, reviewReason, isAutoGenerated, bankInfoSnapshot), `billing_batches`, `billing_batch_items`, `prepaid_accounts`, `prepaid_ledger`, `translation_units`, `translation_unit_logs`, `divisions`, and `translator_sensitive`.
- **Settlement Automation (정산 자동화 1차):** When a task is marked `done`, the system auto-generates a settlement with: (1) payout due date calculation (1–15th → end of month, 16–31st → 15th of next month, KST), (2) settlement type classification (WITHHOLDING_3_3/VAT_INVOICE/OVERSEAS_REMITTANCE/OTHER_REVIEW based on `translator_sensitive.paymentMethod`), (3) tax calculation (3.3% withholding or VAT), (4) auto-status classification (`ready` for domestic 3.3% with bank info; `pending_review` for all other cases), (5) bank info snapshot at time of creation. Admin can approve `pending_review` → `ready`, then pay. New settlements enum: `draft | pending_review | pending | ready | paid`.
