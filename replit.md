...
- **데이터 레이어 1차 (번역 데이터 자산화)**: `translation_units` 테이블(projectId/taskId/sourceText/targetText/sourceLang/targetLang/domain/translatorId/qualityLevel/securityLevel/isAnonymized/anonymized텍스트/글자수/단어수/segmentIndex/status + 6개 인덱스) + `translation_unit_logs` 테이블(변경 이력). `entityTypeEnum`에 "translation_unit" 추가. 서비스: `translationUnitService.ts` (segmentTranslationPair — 줄바꿈/문장부호 분리+fallback, buildTranslationUnitsFromPairs — 세그먼트별 insert+로그, anonymizeTranslationUnit — 정규식 마스킹 [PERSON/COMPANY/EMAIL/PHONE/RRN/URL], bulkAnonymizeByProject, excludeTranslationUnit, rebuildTranslationUnitsForProject). 프로젝트 완료 시 `tryBuildOnProjectComplete` 비치명적 후크 자동 실행. API: GET /admin/translation-units/stats, GET /admin/translation-units (필터+페이지), GET /admin/translation-units/:id (로그 포함), POST /admin/projects/:id/translation-units, POST /admin/projects/:id/translation-units/rebuild, POST /admin/translation-units/:id/anonymize, POST /admin/projects/:id/translation-units/anonymize, PATCH /admin/translation-units/:id/exclude, PATCH /admin/translation-units/:id. 권한: admin/staff=조회가능, admin only=쓰기. AdminDashboard 사이드바에 "데이터 자산" 그룹 + "🗃️ 번역 데이터" 메뉴 추가. UI: 통계카드(총유닛/상태별/언어쌍/도메인) + 다중필터(키워드/프로젝트ID/언어쌍/도메인/보안등급/상태) + 세그먼트 목록 테이블 + 우측 상세패널(원문/번역문/익명화텍스트/메타정보/변경이력) + 익명화/제외 버튼 + 번역쌍 직접추가 폼.
