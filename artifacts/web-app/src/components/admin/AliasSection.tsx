...
// 두 가지 모드를 지원한다(동일 UI·동일 컴포넌트 재사용).
//   1) bound 모드  : basePath 지정 → 서버 CRUD 즉시 반영 (상세 모달 · 수정 화면)
//                    엔드포인트 규약: GET/POST `${basePath}/aliases`, PUT/DELETE `${basePath}/aliases/:aliasId`
//   2) draft 모드  : basePath 미지정 + value/onChange 로 로컬 문자열 목록만 관리 (등록 화면 — 아직 id 없음)
//                    등록 저장 시 상위 폼이 이 목록을 동일 Alias API 로 저장한다.
//
// UI 공통 동작:
//   - Tag 목록 + 각 Tag 우측 ✕ 삭제
...
export function AliasSection({ basePath, token, onToast, value, onChange, compact, headerLabel = "Alias", testId = "alias-section" }: {
  /** 지정 시 bound(서버 CRUD) 모드, 미지정 시 draft(로컬) 모드. 예: `/api/admin/translators/42` */
  basePath?: string;
  token: string;
  ...
  /** 등록/수정 폼용 간결 모드 — 내부 헤더·빈 안내문·배경 박스를 숨기고 Tag만 표시 */
  compact?: boolean;
  /** 비-compact 모드 상단 헤더 라벨 */
  headerLabel?: string;
  /** 루트 컨테이너 data-testid */
  testId?: string;
}) {
  const draft = basePath == null;
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(api(`${basePath}/aliases`), { headers: authH });
      const data = await res.json();
      if (res.ok) setAliases(Array.isArray(data) ? data : []);
      else onToast(`오류: 별칭 조회 실패 (${res.status})`);
    } catch { onToast("오류: 별칭 조회 실패"); }
    finally { setLoading(false); }
  }, [basePath, token, draft]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const submitAdd = async () => {
    ...
    setBusy(true);
    try {
      const res = await fetch(api(`${basePath}/aliases`), {
  ...
  const submitEdit = async (it: DisplayItem) => {
    ...
    setBusy(true);
    try {
      const res = await fetch(api(`${basePath}/aliases/${it.key}`), {
  ...
  const remove = async (it: DisplayItem) => {
    ...
    setBusy(true);
    try {
      const res = await fetch(api(`${basePath}/aliases/${it.key}`), { method: "DELETE", headers: authH });
  ...
  return (
    <div style={containerStyle} data-testid={testId}>
      {/* 간결 모드에서는 상위 폼이 라벨을 제공하므로 내부 헤더를 숨긴다. */}
      {!compact && (
