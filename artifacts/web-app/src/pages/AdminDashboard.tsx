                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr>
                        ...
                        {["ID","프로젝트","통번역사","원금액","원천세","실지급액","정산 유형","지급 예정일","상태","액션"].map(h => <th key={h} style={tableTh}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredSettlements.map(s => {
                        ...
                        return (
                          <tr key={s.id}
                            ...
                            {/* ID */}
                            <td style={{ ...tableTd, color: "#9ca3af", fontSize: 12 }}>
                              #{s.id}
                              {s.isAutoGenerated && <div style={{ fontSize: 10, color: "#7c3aed", fontWeight: 600, marginTop: 2 }}>🤖 자동</div>}
                            </td>
                            {/* 프로젝트 */}
                            ...
                            <td style={{ ...tableTd, fontSize: 12, whiteSpace: "nowrap" }}>
                              {s.payoutDueDate
                                ? <span style={{ fontWeight: isOverdue || isDueToday ? 700 : 400, color: isOverdue ? "#dc2626" : isDueToday ? "#d97706" : "#374151" }}>
                                    {isOverdue && "⚠ "}{s.payoutDueDate}
                                  </span>
                                : <span style={{ color: "#d1d5db" }}>—</span>}
                            </td>
                            {/* 상태 */}
