...
export function AdminDashboard({ user, token, permissions = [], onLogout }: { user: User; token: string; permissions?: string[]; onLogout?: () => void }) {
  ...
  return (
    <>
      ...
      {/* ── 정산 탭 (사이드바 하위메뉴: 지급회차 관리 · 지급명세서 · 세무자료) ── */}
      {(adminTab === "settlements" || adminTab === "settlement-statement" || adminTab === "settlement-tax") && (
        <SettlementTabs
          view={adminTab === "settlement-statement" ? "statement" : adminTab === "settlement-tax" ? "tax" : "payout"}
          token={token ?? ""}
          onToast={setToast}
        />
      )}
