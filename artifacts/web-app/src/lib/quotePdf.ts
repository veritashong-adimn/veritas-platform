...
function buildDetailText(item: QuoteDetailItem): string {
  ...
  // 할인 항목 — 방식만 표기(사유는 내부용, 미출력). 예: "5% 할인" / "금액 할인"
  if (type === 'discount') {
    return item.discountType === 'percent'
      ? `${Number(item.discountValue ?? 0)}% 할인`
      : '금액 할인';
  }
...
function detectServiceType(items: QuoteDetailItem[]): string {
  ...
  const types = new Set(items.map(it => it.itemType ?? 'translation').filter(t => t !== 'expense' && t !== 'discount'));
  if (types.size === 1) {
    const t = [...types][0];
