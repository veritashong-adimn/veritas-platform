...
/**
 * 원화 금액 표시 포맷(플랫폼 공통 Utility). 화면 표시 전용이며 계산/저장 값은 변경하지 않는다.
 * 기존 화면들의 인라인 구현(`{n.toLocaleString()}원`)과 출력이 100% 동일하도록 유지한다:
 *   - 기본 locale 기준 천단위 콤마 (locale 미지정)
 *   - 반올림·가공 없이 전달된 숫자를 그대로 표기
 *   - 끝에 "원" 접미
